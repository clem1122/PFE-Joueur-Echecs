...
% 0. Paramètres
percentage_threshold = 25; % Seuil pour le pourcentage de différence
...
% Charger l'image de référence (échiquier vide)
referenceImagePath = 'empty.png';
referenceImage = imread(referenceImagePath);
...
for i = 1:(length(imageFiles)-1)
    ...
    movement = detectMove(img1, img2, cases, tform, outputSize, ...
                          percentage_threshold, sensitivityThreshold);
    fprintf('Mouvement détecté entre %s et %s : %s\n', ...
        imageFiles(i).name, imageFiles(i+1).name, movement);
end